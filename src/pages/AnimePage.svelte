<script lang="ts">
  import { itemsStore, loadingAnimeStore } from "../lib/store";
  ...
  $: rawItems = $itemsStore.filter((item) => item.category === "anime");
  ...
  $: filteredItems = rawItems
    ...
    .sort((a, b) => {
      const scoreA = a.score ?? (a.rating ? a.rating * 2 : -1);
      const scoreB = b.score ?? (b.rating ? b.rating * 2 : -1);
...
<section class="py-10 md:py-24 px-3 sm:px-6 lg:px-8 max-w-6xl mx-auto">
  <div class="relative w-full h-40 md:h-56 mb-8 overflow-hidden rounded-lg">
    <img
      loading="lazy"
      src="/section/animeSection.png"
      alt="Anime Section"
      class="absolute inset-0 w-full h-full object-contain object-right"
    />
    <div
      class="absolute inset-0 flex pointer-events-none justify-end pr-8 pt-0 items-start"
    >
      <span
  ...
  <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
    {#if $loadingAnimeStore && rawItems.length === 0}
      {#each Array(8) as _}
        <ArchiveCardSkeleton />
      {/each}
